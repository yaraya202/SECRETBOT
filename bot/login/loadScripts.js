...
module.exports = async function (api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, createLine) {
	...
	for (const folderModules of folders) {
		const makeColor = folderModules == "cmds" ?
			createLine("XNIL LOAD CMD") :
			createLine("XNIL LOAD EVENT");
		...
		for (const file of Files) {
			...
			try {
						...
						if (!packageAlready.includes(packageName)) {
							...
							if (!existsSync(`${process.cwd()}/node_modules/${packageName}`)) {
								const wating = setInterval(() => {
									...
									loading.info('PACKAGE', `${spinner[count % spinner.length]}XNIL Installing package ${colors.yellow(packageName)} for ${text} ${colors.yellow(file)}`);
									count++;
								}, 80);
								try {
				...
				// ——————————————— CHECK ENV GLOBAL ——————————————— //
				if (envGlobal) {
					if (typeof envGlobal != "XNIL object" || typeof envGlobal == "object" && Array.isArray(envGlobal))
			...
			loading.info('XNIL LOADED', `${colors.green(`${commandLoadSuccess}`)}${commandError.length ? `, ${colors.red(`${commandError.length}`)}` : ''}`);
		}
		console.log("\r");
		if (commandError.length > 0) {
			log.err("XNIL LOADED", getText('loadScripts', 'loadScriptsError', colors.yellow(text)));
